...
class StyledGenerator(nn.Module):
    ...
    def forward_from_style(self, style, noise=None, step=0, alpha=-1, mean_style=None, style_weight=0.0, mixing_range=(-1, -1)):
        batch = style.shape[0]
...
if __name__ == "__main__":
    ...
    mean_style = get_mean_style(G, torch.device('cpu'), style_mean_num=10)
    img_size = 256
    step = int(math.log(img_size, 2)) - 2
    alpha = 1
    style_weight = 0.7

    z_latent = torch.randn(size=(1, 512))
    style = G.get_style(z_latent)
    print(style.shape)

    image = G.forward_from_style(style, step=step, alpha=alpha, mean_style=mean_style, style_weight=style_weight)
    print(image.shape)
